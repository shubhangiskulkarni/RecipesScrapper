...
public class HypothyroidismAfterElimnatngAllergies {
	private static WebDriver driver;
	List<String> hypothyroidism_EliminateItem = new ArrayList<String>();
	List<String> hypothyroidism_AllergiesItem = new ArrayList<String>();
	String ingre_List = "";
	String rec_Category;
	String fud_Category;
	String morbid_Condition;

	public HypothyroidismAfterElimnatngAllergies(WebDriver driver) {
	...
	public void readExcel() {
		...
		for (int i = 2; i <= 14; i++) {
			String elimntdAllergiesData = reader.getCellData("Diabetes-Hypothyroidism-Hyperte", 0, i);
			hypothyroidism_AllergiesItem.add(elimntdAllergiesData);
	...
	public void gettingRecipes() throws InterruptedException, IOException {
		...
		for (int p = 1; p <= noOfPages; p++) {
			...
			for (int i = 1; i <= rec_Size; i++) {
				...
				Boolean isElimntdAllrgyRecipe = isElimntdAllrgyRecipe(hypothyroidism_AllergiesItem, ingre_List);
				if (validRecipe) {
		...
		XSSFSheet sheet = workbook.createSheet("HypothyroidismRec_EliminatngAllergyItems-");
		int rowCount = 0;
		...
		String filePath = ".\\target\\hypothyroidismRecWithNoAllergyItems.xlsx";
	...
	public boolean isElimntdAllrgyRecipe(List<String> hypothyroidism_AllergiesItem, String ingr_List) {
		// System.out.println(ingr_List);
		String[] ingredients = ingr_List.split(",");
		for (String ingr : ingredients) {
			ingr = ingr.trim();
			if (ingr.length() > 0 && hypothyroidism_AllergiesItem.contains(ingr)) {
				System.out.println(ingr);
				System.out.println("Invalid Item");
				return false;
			}
